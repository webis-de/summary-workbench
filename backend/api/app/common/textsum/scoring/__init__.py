...
class Scorer(object):
    ...
    def _count_common_words(
        self, words_sentence_one: List[str], words_sentence_two: List[str]
    ) -> int:
        return len(set(words_sentence_one) & set(words_sentence_two))

    def entity_overlap(
        self,
        sentence_1_entities: Set[Tuple[str, str]],
        sentence_2_entities: Set[Tuple[str, str]],
    ) -> int:
