...
class LoBARTModel(object):
    ...
    def summarize(self, text:str=None, ratio:float=0.2):
        ...
        summary_ids = self.model.generate(token_ids, num_beams = num_beams, length_penalty=length_penalty, max_length=max_length, min_length=min_length,no_repeat_ngram_size=no_repeat_ngram_size)
        summary_text = self.tokenizer.decode(summary_ids.squeeze(), skip_special_tokens=True).strip()
        return summary_text         
