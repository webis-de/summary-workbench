...
function Settings(prop) {
  ...
  useEffect(() => {
    ...
    .then(response => response.json()).then(result => setSettings(result));
  }, []);
  ...
  const onClick = (metric) => {
    ...
    fetch("http://localhost:5000/api/setting", {method, body, headers: {'Content-Type': 'application/json'}})
    .then(() => {newsettings[metric].is_set = is_set; setSettings(newsettings)})
      ...
      <Card.Body>
        <ButtonGroup className="d-flex">
          {Object.keys(settings).map(metric => {
            ...
            return <Button className="border-dark" key={metric}
